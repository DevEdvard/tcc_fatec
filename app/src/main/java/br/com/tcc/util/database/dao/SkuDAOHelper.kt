...
//    fun insertSkuTeste()
    ...
    @Insert
    fun insert(produto: ArrayList<Sku>)

    @Insert
    fun insert(produto: Sku)

    @Query("SELECT * FROM SKU WHERE CODLOJA = :id AND FLCOLETADO NOT IN (1)")
    fun selectListaSku(id: Int): List<Sku>

    @Query("SELECT * FROM SKU WHERE CODLOJA = :codLoja")
    fun selectListaSkuColeta(codLoja: Int): List<Sku>
    ...
    @Query("SELECT * FROM SKU WHERE CODLOJA = :codLoja AND ID NOT IN (SELECT IDPRODUTO FROM COLETAPRODUTO)")
    fun selectModuloColetado(codLoja: Int) : List<Sku>
